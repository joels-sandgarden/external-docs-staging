...
That status makes the difference between a Publication that can accept work and one that needs attention easy to spot. When health is unhealthy, the details page shows a retry action and troubleshooting guidance so the next check stays visible and easy to understand. The result is a simple scan for the list view and a deeper explanation for the details view.
