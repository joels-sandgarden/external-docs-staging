...
1. Get a key from a supported provider that supports real inference. A key that can only list models fails verification.
2. Add the key in the flow that matches the setup path.
...
See [AI provider keys](./h3-ai-provider-keys.md) for the reference and [Billing and plans](./h4-billing-and-plans.md) for plan coverage.