...
url: "docs/how-this-site-is-made"
description: "These docs are written and maintained by Doc Holiday itself."
---
...
1. **Fact-check.** Every claim on the drafted page — every screen label, status name, configuration field — is verified against the product source before the work is approved. Pages that fail are corrected the same way they were written: the exact corrections go back to Doc Holiday as a new request, so the page and its history stay in one pipeline.
...
People still review every page, take the screenshots, decide the site's structure, and write this meta page you're reading. Doc Holiday does the part humans can't keep up with: reading the source and keeping the words true to it.
