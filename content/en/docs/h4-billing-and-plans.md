---
title: Billing and Plans
url: "docs/billing-and-plans"
description: "Plan access, billing status, eligibility, and trial checkout behavior."
---

Open **Settings → Billing** to review the available plans and what each one includes. The page shows the current billing options for the organization. Admins on the Bring Your Own Key and Open Source plans can access it. Enterprise billing stays with the account team.

![The Billing page with the three plan cards](/screenshots/billing-and-plans/tier-cards.png)

## Open Source Plan

Our Open Source Plan is free for public open source projects and runs on the organization's own provider key for inference. See [Add a key](./b6-add-your-openai-key.md) and [AI provider keys](./h3-ai-provider-keys.md) for setup context.

Open Source organizations can apply for OSS access from the Billing page. The application asks for the project name, public repository URL, and description, and Doc Holiday sends the decision by email.

Approved OSS organizations can access Billing and keep account routing aligned with the OSS plan.

## Bring Your Own Key

Bring Your Own Key costs $100/month. Eligible organizations can start with a free trial, and Stripe only collects a payment method when it needs one. If no payment method is on file when the trial ends, Stripe ends the trial subscription. Billing stays under the organization's account portal.

Doc Holiday pauses gated flows while billing is not current.

Bring Your Own Key eligibility depends on customer type. The Billing page shows the plan only when the organization qualifies.

Use [Add a key](./b6-add-your-openai-key.md) and [AI provider keys](./h3-ai-provider-keys.md) for key setup and management.

## Enterprise

Please book a demo at https://scheduler.zoom.us/elizabeth-zalman/doc_holiday_demo — inference is managed for you.