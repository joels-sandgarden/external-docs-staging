...
description: "Confluence as read-only context, plus app-backed and read-only Jira setup paths."
---

Use this guide when Doc Holiday needs Confluence for read-only context or Jira source setup that reacts to Publication triggers. For a broader Sources overview, see [Sources](./c1-sources.md).
...
3. If the form includes a pause option, use it for temporary stops instead of removing the Source.
4. Select **Save Changes**.

## Jira app-backed setup

Note: Both Jira paths use the Publication triggers **Send to Doc** and **Issue status changes**. Configure those triggers on the Publication, then open [Configure triggers](./d2-configure-triggers.md).
...
7. If the form includes a pause option, use it for temporary stops instead of removing the Source.
